...
- **代码编辑**: Monaco Editor 内核，支持语法高亮、多标签页、分栏编辑、自动语言检测
- **文件管理**: 文件资源管理器支持文件/文件夹创建、删除、重命名、刷新、折叠与最近项目管理
- **集成终端**: 基于 PTY 的真实终端，支持默认 shell、工作区 cwd、自适应大小与多终端标签
- **外观设置**: 全局主题切换、字号调节，并提供快捷键与状态栏入口
...
当前项目已经覆盖了常见的 IDE 核心闭环：

- 左侧文件树与最近工作区/最近文件管理
- 中央 Monaco 编辑区与多标签、分栏、跳转定位
- 右侧 AI 面板，支持对话、工具调用和流式输出
- 终端、搜索、Git、设置、状态栏等基础工作台能力
- 主题与字号可全局统一调整，便于现场演示和长期使用
...
如果你需要在最终验收时快速说明项目价值，建议重点展示以下能力：

1. 打开工作区后，先展示左侧 Explorer、最近工作区/文件、以及折叠与删除能力。
2. 打开一个文件，演示 Monaco 编辑、多标签切换、分栏编辑和跳转定位。
3. 打开终端，说明它现在是 PTY 终端，并演示在当前工作区 cwd 下执行命令。
4. 使用状态栏或快捷键切换主题、调整字号，说明全局外观已经统一。
5. 打开 AI 面板，演示一次带工具调用的对话，例如读取文件、搜索代码或执行命令。
6. 切到 Git 面板，演示状态查看、Diff、暂存或提交入口。

建议验收前先执行一次 `npm run build`，确认当前版本可以稳定打包。
